...
import json # Used for json.dumps, though jsonify typically handles it

# Attempt to import EmailGuardAI from the installed SDK package
try:
    from email_guard_sdk.classifier import EmailGuardAI
except ImportError as e:
    # Log an error if the SDK cannot be imported, crucial for debugging deployment
    ...
    # In a production environment, you might want to exit or fail health checks
    email_guardian = None # Indicate that the AI model is not available
    # It's good practice to try to load the model later during request if it failed at startup
    # or to make the app's health check reflect this.
    # For now, we'll let it proceed but responses will reflect the error.

# Initialize Flask app
app = Flask(__name__)
# Enable CORS for frontend communication.
# In production, you'd specify exact origins (e.g., CORS(app, origins=["https://your-frontend-url.vercel.app"]))
CORS(app) 

# Initialize the AI model
# The EmailGuardAI class handles the singleton pattern internally,
# so calling it repeatedly will return the same instance.
if 'email_guardian' not in locals() or email_guardian is None:
    try:
        ...
        email_guardian = None # Set to None if loading failed

# In-memory storage for scan history (for demonstration purposes)
SCAN_HISTORY = []

# Get API key from environment variable
# If API_KEY is not set, authentication will be skipped (for development/testing flexibility)
...
def authenticate_api_key():
    if API_KEY: # Only enforce authentication if API_KEY is set in environment
    ...
    return True, None, None # Authentication successful or not required
...
def scan_email():
    ...
    try:
        ...
        # Store a simplified version of the email in history
        history_entry = {
            "email_preview": (email_text[:100] + "...") if len(email_text) > 100 else email_text,
            "classification": result['classification'],
            "confidence": result['confidence'], # Include confidence in history for more detail
            "timestamp": result['timestamp']
        }
        SCAN_HISTORY.insert(0, history_entry)
        
        # Keep history limited to prevent excessive memory usage in a production app
...
if __name__ == '__main__':
    ...
    # In production, Gunicorn (or your chosen WSGI server) will run the app.
